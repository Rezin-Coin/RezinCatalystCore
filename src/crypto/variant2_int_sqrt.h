...
#define VARIANT2_INTEGER_MATH_SQRT_STEP_REF() sqrt_result = integer_square_root_v2(sqrt_input)
...
// 1) Write it as x = 1.NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN000... in binary (1 <= x < 2,
// all 64 bits of n are used) 2) Calculate sqrt(x) = 1.0RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR... (1 <= sqrt(x) < sqrt(2), so
// it will always start with "1.0" in binary) 3) Take 32 bits that come after "1.0" and return them as a 32-bit unsigned
// integer, discard all remaining bits
//
...
/*
VARIANT2_INTEGER_MATH_SQRT_FIXUP checks that "r" is an integer part of "sqrt(2^64 + sqrt_input) * 2 - 2^33" and adds or
subtracts 1 if needed It's hard to understand how it works, so here is a full calculation of formulas used in
...
b can be 0 or 1
If b is 0 then we need to compare "r2 + 2^32 + 1/4 <= sqrt_input - s" which is equal to "r2 + 2^32 < sqrt_input - s"
because all numbers here are integers If b is 1 then (b+1)^2/4 = 1, so we need to compare "r2 + 2^32 + 1 <= sqrt_input -
s" which is also equal to "r2 + 2^32 < sqrt_input - s"
-----------------------------------------------------------------------------------
...
This inequality is true if sqrt_input > 1 and it's easy to check that s = 0 if sqrt_input is 0 or 1, so there will be no
integer overflow
*/
