...
ValidateTransaction::ValidateTransaction(
    ...
    m_transactionStopHeight(
        CryptoNote::parameters::CRYPTONOTE_STOP_BLOCK_NUMBER
        - CryptoNote::parameters::CRYPTONOTE_STOP_TX_X_BLOCKS_BEFORE - 1)
{
}

TransactionValidationResult ValidateTransaction::validate()
{
    /* Validate that the transaction is before the final stopping window */
    if (!validateTransactionIsBeforeStopWindow())
    {
            ...
            if (!check_key(boost::get<CryptoNote::KeyOutput>(output.target).key))
            {
                setTransactionValidationResult(
                    ...
                    "Transaction output has an invalid output key: "
                        + Common::podToHex(boost::get<CryptoNote::KeyOutput>(output.target).key));
...
bool ValidateTransaction::validateTransactionInputsExpensive()
{
    ...
    for (const auto &input : m_transaction.inputs)
    {
        ...
        validationResult.push_back(m_threadPool.addJob(
            [inputIndex, &input, &prefixHash, &cancelValidation, this]
            {
                const CryptoNote::KeyInput &in = boost::get<CryptoNote::KeyInput>(input);
                if (cancelValidation)
                {
...
bool ValidateTransaction::validateTransactionIsBeforeStopWindow()
{
    if (m_blockHeight >= m_transactionStopHeight)
    {
        const auto remainder = CryptoNote::parameters::CRYPTONOTE_STOP_BLOCK_NUMBER - m_blockHeight - 1;

        setTransactionValidationResult(
            CryptoNote::error::TransactionValidationError::CHAIN_IS_HALTING,
            "Chain will be halting in " + std::to_string(remainder)
                + " blocks, adding new transactions is no longer permitted. " + "So long and thanks for all the fish!");
